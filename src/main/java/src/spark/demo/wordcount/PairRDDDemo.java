...
public class PairRDDDemo {
	...
	/**
	 * 批量更改value的值（和mapValues是有区别的） 官方介绍：对 pair RDD 中的每个值应用 一个返回迭代器的函数，然后
	 * 对返回的每个元素都生成一个 对应原键的键值对记录。通常 用于符号化
	 * 
	 * @Title: flatMapValues
	 * @param pairs
	 */
	...
	public static JavaPairRDD<Integer, Integer> createPairRDD() {
		...
		JavaRDD<Integer> rdd = sc.parallelize(list, 2); // 这个是分区用了，指定创建得到的 RDD
														// 分区个数为 2。
		PairFunction<Integer, Integer, Integer> keyData = new PairFunction<Integer, Integer, Integer>() {
			...
			public Tuple2<Integer, Integer> call(Integer x) throws Exception {
				return new Tuple2<Integer, Integer>(x, x + 1);// 键值对转换,key=x
																// ,value=x+1
			}
		};
	...
	/**
	 * 
	 * 模拟好友精准推荐 找出共同好友 然后共同好友互相推荐 A:C,D,F,E,O B:A,C,E,K,Z C:F,A,D,I D:A,E,F,L
	 * E:B,C,D,M,L F:A,B,C,D,E,O,M G:A,C,D,E,F H:A,C,D,E,O I:A,O J:B,O K:A,C,D
	 * L:D,E,F M:F,G O:A,H,I,J
	 * 
	 * @return
	 */
	private static JavaPairRDD<String, String> createFriendsPairRDD() {
		List<String> list = Arrays.asList("A:C,D,F,E,O", "B:A,C,E,K,Z", "C:F,A,D,I", "D:A,E,F,L", "E:B,C,D,M,L",
				"F:A,B,C,D,E,O,M", "G:A,C,D,E,F", "H:A,C,D,E,O", "I:A,O", "J:B,O", "K:A,C,D", "L:D,E,F", "M:F,G",
				"O:A,H,I,J");
		SparkConf conf = new SparkConf().setMaster("local").setAppName("PairRDDDemo");
		sc = new JavaSparkContext(conf);
		sc.setLogLevel("ERROR");
		...
		JavaPairRDD<String, String> reduceStep1 = keyStep1.reduceByKey((String i1, String i2) -> (i1 + "&" + i2));
